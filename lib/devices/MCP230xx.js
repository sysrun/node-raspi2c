...
MCP230xx.prototype.inputAll = function(cb) {
  var _self = this;
  _self._readGPIO(function(err, value) {
    if (err) return cb(err);
    var pins = [];
    for (var i = 0; i < _self.num_gpios; i++) {
      pins[i] = !!(value & (1 << i));
    }
    cb(null, pins);
  });
...
MCP230xx.prototype.input = function(pins, cb) {
  var _self = this;
  step(
    function inputAll() {
      _self.inputAll(this);
    },
    function processResults(err, vals) {
      if (!Array.isArray(pins)) {
        return cb(null, vals[pins]);
      } else {
        var result = [];
        for (var i = 0, l = pins.length; i < l; i++) {
          result.push(vals[pins[i]]);
        }
        cb(null, result);
      }
    }
...
MCP230xx.prototype._readGPIO = function(cb) {
  ...
  step(
    function readRegisterA() {
      _self._readU8(MCP23017_GPIOA, this);
    },
    function registerARead(err, lower) {
      if (err) return cb(err);
      value += lower;
      this();
    },
    function readRegisterB() {
      if (_self.num_gpios > 8) {
        _self._readU8(MCP23017_GPIOB, this);
      } else {
        this();
      }
    },
    function registerBRead(err, upper) {
      if (err) return cb(err);
      if (upper) {
        value += (upper << 8);
      }
